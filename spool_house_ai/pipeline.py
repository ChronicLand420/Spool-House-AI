...
def _write_job_settings(path: Path, config: AppConfig) -> None:
    lines = [
        ...
        "printer:",
        f"  profile_name: {config.printer_profile.profile_name}",
        f"  printer_type: {config.printer_profile.printer_type}",
        f"  nozzle_diameter_mm: {config.printer_profile.nozzle_diameter_mm}",
        f"  line_width_mm: {config.printer_profile.line_width_mm}",
        f"  layer_height_mm: {config.printer_profile.layer_height_mm}",
        f"  first_layer_height_mm: {config.printer_profile.first_layer_height_mm}",
        "printability:",
        f"  use_printer_aware_defaults: {str(config.printability.use_printer_aware_defaults).lower()}",
    ...
    *,
    ...
    payload = {
        ...
        "dimensions": {
            ...
            "generic_3mf_export": "automatic",
            "printer_profile_name": config.printer_profile.profile_name,
            "printer_type": config.printer_profile.printer_type,
            "printer_nozzle_diameter_mm": config.printer_profile.nozzle_diameter_mm,
            "printer_line_width_mm": config.printer_profile.line_width_mm,
            "printer_layer_height_mm": config.printer_profile.layer_height_mm,
            "printer_first_layer_height_mm": config.printer_profile.first_layer_height_mm,
            "use_printer_aware_printability_defaults": config.printability.use_printer_aware_defaults,
        ...
        "settings_used": {
            ...
            "printer_profile": asdict(config.printer_profile),
...
def _write_job_summary(path: Path, status: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "## Print-Safe Cleanup",
        f"- Enabled: `{printability.get('enforcement_enabled', False)}`",
        f"- Printer-aware defaults: `{(printability.get('thresholds') or {}).get('use_printer_aware_defaults', False)}`",
        f"- Generation paths: `{printability.get('paths_invoked', [])}`",
        f"- Minimum detail width mm: `{(printability.get('thresholds') or {}).get('minimum_feature_width_mm', '')}`",
        f"- Shortest usable stroke mm: `{(printability.get('thresholds') or {}).get('minimum_segment_length_mm', '')}`",
        f"- Remove pieces under mm2: `{(printability.get('thresholds') or {}).get('minimum_island_area_mm2', '')}`",

...
class GuiRecommendationTests(unittest.TestCase):
    ...
    def test_filament_relief_quality_and_solid_base_controls_feed_config(self) -> None:
        ...
        try:
            ...
            self.assertTrue(window.filament_quick_group.isVisible())
            self.assertTrue(window.filament_detail_quality.isVisible())
            self.assertTrue(window.filament_detail_quality.isEnabled())
            self.assertFalse(window.detail_mode.isEnabled())
            self.assertFalse(window.detail_mode.isVisible())
            self.assertFalse(window.output_scale.isEnabled())
            self.assertFalse(window.output_scale.isVisible())
            self.assertEqual(window._combo_value(window.filament_detail_quality), "700000")
            self.assertEqual(window._combo_value(window.filament_relief_style), "auto")
            auto_index = window.filament_relief_style.findData("auto")
            self.assertGreaterEqual(auto_index, 0)
            silhouette_index = window.filament_relief_style.findData("silhouette_outline")
            self.assertGreaterEqual(silhouette_index, 0)
            ...
            window.filament_relief_style.setCurrentIndex(silhouette_index)
            ...
            self.assertEqual(config.filament_swap_relief.relief_style, "silhouette_outline")
